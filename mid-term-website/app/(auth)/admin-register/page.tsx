...
export default function AdminRegisterPage() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!res.ok) {
        const msg = typeof data.error === "string" ? data.error : "Admin signup failed";
        toast.error(msg);
        return;
      }
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="mt-2 text-muted-foreground">Register admin credentials before admin login.</p>
      </div>
      ...
      <p className="mt-6 text-center text-sm text-muted-foreground">
        Already have admin credentials? <Link href="/admin-login" className="text-primary hover:underline">Sign in</Link>
      </p>
    </div>
  );
}
