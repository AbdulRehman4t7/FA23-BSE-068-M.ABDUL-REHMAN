...
export async function POST(req: Request) {
  try {
    ...
    if (error || !newUser) {
      return NextResponse.json({ error: 'Failed to create moderator account' }, { status: 500 });
    }

    try {
      await db.from('profiles').upsert({ id: newUser.id, email: newUser.email, role: 'moderator' });
    } catch {}
    ...
    const response = NextResponse.json(
      {
        ...
        user: { id: newUser.id, name: newUser.name, email: newUser.email, role: 'MODERATOR' },
        token,
      },
      { status: 201 }
    );
